...
## Data / Dataset Feasibility — which ideas materialize easily

*Added after the ideation pass: filters the 9 ideas by how easily the data/datasets needed to actually build them can be obtained, given the SIA task-folder contract and each idea's own goal. Grounded against the shipped tasks (verified in-repo, not inferred).*

### Shipped-task data already in the repo

All four shipped tasks carry full public **and** private splits with an `accuracy` grader — so any idea that reuses them needs **zero dataset authoring**:

| Task | Public items | Private items | Metric | Note for ideas |
|---|---:|---:|---|---|
| `gpqa` | 198 | 198 | accuracy | MCQ; `results.json` schema also emits token counts (anchors F2) |
| `lawbench` | 914 | 914 (+5,333 train) | accuracy | **largest set** — the one with enough data to split for R1 |
| `longcot-chess` | 50 | 50 | accuracy | too small for held-out splitting |
| `spaceship-titanic` | 7,824 (train) | 871 | accuracy | Kaggle binary classification |

> **Gap:** no `runs/` directory exists yet, so ideas that *analyze run output* (R2, R3) need at least one seed run before there is data to analyze. Their "data" is self-generated by running the loop, not externally sourced.

### Tiers by data-acquisition effort

**Tier D1 — data already in-repo, zero authoring.** *(easiest to materialize)*
- **F1 Archive Mode**, **F2 Cost-Aware Loop**, **F3 Parallel Fan-out** — pure framework changes that run *on top of* the shipped tasks. No new dataset at all; the loop already produces everything they consume.
- **R1 Held-Out Verifier** — needs only a *split* of an existing private set, not new data. `lawbench`'s 914 private items split 70/30 (~640/274) is large enough for a **stable** train-vs-held-out estimate — which is precisely the idea's goal (a noisy split can't show "the gap widens"). `gpqa` (198) works but is borderline; `longcot-chess` (50) is too small. **Best data-vs-goal fit in the set.**

**Tier D2 — data self-generated by running the loop** (no external data; compute is the only cost):
- **R2 Improvement Taxonomy** — analyzes `improvement.md` files; needs just **one** multi-gen run to seed, then pure post-hoc analysis.
- **R3 Efficiency Curve** — same source, but needs **many** long runs → compute-bound, not data-bound. Cheapest tasks + small models keep it affordable.

**Tier D3 — easy public dataset, no hand-labeling:**
- **A2 SIA-for-SQL** — mature public NL→SQL data exists off-the-shelf (Spider, WikiSQL, or the `chinook` sample DB this proposal already names). Execution-based grading compares result sets, so **zero hand-labeling** is required. Strongest Applied idea on data-availability, and it fits the task-folder contract cleanly (the SQLite file is a read-only `data/` mount; `evaluate.py` executes candidate SQL).

**Tier D4 — data exists but needs reshaping or labeling** (real friction):
- **A1 Regex-Golf** — public IE/NER corpora exist (CoNLL-2003, i2b2/n2c2 clinical) but must be remapped into the target triple format and paired with an F1 grader; or ~80 sentences hand-labeled. The grader is small, the labeling/remapping is the cost.
- **A3 SIA Triages Itself** — no external dataset, but failure logs must be *induced* (failures are rarer than successes) and then hand-labeled into classes. Self-sourced, but not free.

### Data-feasibility summary

| # | Idea | Data source | Authoring effort | Enough data for its goal? |
|---|------|------|:---:|---|
| F1 | Archive Mode | shipped tasks (reuse) | **none** | yes — A/B on `gpqa`/`lawbench` |
| F2 | Cost-Aware Loop | shipped tasks (reuse) | **none** | yes — `gpqa` emits token counts |
| F3 | Parallel Fan-out | shipped tasks (reuse) | **none** | yes — any shipped task |
| R1 | Held-Out Verifier | split of existing private set | **trivial (split only)** | **yes — `lawbench` 914 is large enough** |
| R2 | Improvement Taxonomy | self-generated (1 run) | low (run once) | yes after a seed run |
| R3 | Efficiency Curve | self-generated (many runs) | low data / high compute | yes — compute-bound, not data-bound |
| A2 | SIA-for-SQL | public NL→SQL (Spider/WikiSQL/chinook) | low (no labeling) | yes — off-the-shelf |
| A1 | Regex-Golf | public IE corpora **or** hand-labeled | medium (remap/label) | yes once reshaped |
| A3 | Log classifier | self-induced failure logs | medium (induce + label) | needs enough induced failures |

**Recommendation (data-readiness lens):** the ideas you can put data behind *today* are **F1, F2, F3, R1** (in-repo data, nothing to author) and **A2** (off-the-shelf public SQL data, no labeling). Among them **R1 is the standout** — the data is already present *and* `lawbench` is sized to actually demonstrate the idea's goal. R2/R3 are data-trivial but gated on running the loop first; A1/A3 are the only ideas with genuine dataset-authoring cost.
